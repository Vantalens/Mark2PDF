...
| PNG | 作为图片资源进入 AssetStore | P4 输出轻量栅格摘要 PNG | 不等同于浏览器排版截图 |
| PDF | 文本型 PDF 提取 | P4 程序化生成二进制 PDF | 复杂分页、字体嵌入、图片和表格绘制仍会降级 |
...
- PNG/JPEG 输出：真实视觉渲染器完成前不进入输出矩阵；禁止用空白图、单像素图或摘要栅格冒充文档图片输出。
