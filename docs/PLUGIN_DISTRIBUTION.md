...
## External Engine Bridge 插件（P8-M5）

`kind: "engine-bridge"` 是 P8 多模型架构引入的新插件类型，用于在桌面环境（Tauri sidecar）下调用本地引擎（LibreOffice / Pandoc / Calibre / ofdrw 等），把 `from -> to` 之间的转换从核心 mapper 链替换为高保真直达路径。详见 [CONVERSION_ROUTING.md](CONVERSION_ROUTING.md) 和 [MULTI_MODEL_ARCHITECTURE.md](MULTI_MODEL_ARCHITECTURE.md)。

manifest 字段：

- `kind: "engine-bridge"`
- `bridges[]`：每条桥声明 `{ from, to, engine, command?, lossLevel? }`。
- `requiresLocalBinary`：依赖的本地二进制名（如 `soffice` / `pandoc`），桌面端在加载前探测；浏览器端不暴露。
- `securityScope[]`：精确权限范围，例如 `sidecar:soffice` / `fs-readonly:user-temp` / `cmap:bundled`，沙箱据此审计。

行为：

- 装了 bridge 插件且本地有对应二进制，Route Planner 在 `from -> to` 上直接给出 hot 温度，绕过中间 mapper 链。
- bridge 调用失败（找不到二进制 / 超时 / 输出非法）自动回落到核心 mapper 路径，发 `BRIDGE_FALLBACK` warning，不阻塞主流程。
- bridge 调用全程 local-only：禁止网络权限、禁止读取 bridge 范围之外的文件、调用前后写 provenance / qualityReport。
- 浏览器端不加载 engine-bridge 插件：`navigator.userAgent` 检测到 Tauri 环境才允许执行。
- 浏览器端的 Plugin Workbench 仍然可以展示 bridge 的形态和能力，但不允许"启用"按钮。

bridge 不替代核心 reader / writer：核心 reader / writer 仍按 SemanticDoc / WorkbookModel / SlideModel / FixedLayoutModel 流转，bridge 只是给"高保真高频路径"提供 fast-path。
