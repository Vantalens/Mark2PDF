...
DOCX、PPTX、XLSX、PDF input、EPUB 已作为热门输入能力进入基础包；DOCX/XLSX/EPUB/PPTX/PDF 输出已建立本地基线。前端按输入格式筛选输出，PPTX 不对 DOCX 等正文文档源开放，PNG/JPEG 视觉渲染未达标前不进入输出矩阵。高保真渲染、OCR、本地模型、OFD 和更重的格式增强仍必须走按需插件。OFD 是战略攻坚格式，不是默认核心包格式。
