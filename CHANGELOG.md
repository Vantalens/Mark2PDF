...
- 👁️ **实时预览**：自动或手动刷新渲染预览
- 🖊️ **预览编辑模式**：直接在预览中编辑内容并自动同步源码
- ⌨️ **快捷键**：`Ctrl+S` / `Cmd+S` 快速生成 PDF
- 📊 **字数统计**：实时显示文档字数和行数
...
- 🖥️ Electron 桌面应用（Windows portable exe）
...
- ✅ **按需加载**：renderer 模块从启动时加载改为首次使用时才加载
- ✅ **Playwright 懒加载**：Chromium 仅在首次导出 PDF 时加载
- ✅ **无阻塞首屏**：启动时先显示空界面，不触发预览渲染
- ✅ **启动耗时日志**：完整的 `[STARTUP]` 性能日志用于调试
- ✅ **单实例约束**：防止多进程争用端口与资源
