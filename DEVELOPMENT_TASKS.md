...
- [ ] Worker 阶段进度：progress 从固定 20%/100% 改为 read / parse / validate / convert / render / package。
- [ ] 可视化进度条：显示阶段、百分比、忙碌状态和取消状态。
...
5. 然后补 AI-ready Markdown 输出准则、warnings 和质量评分维度，让降级结果可解释。
6. 再设计可编辑 `DocumentModel` 与结构化编辑器，形成网页端动态编辑和实时预览壁垒。
7. 进入 ZIP/OOXML 基础设施，优先做 DOCX/PPTX/XLSX 输入到 Markdown/HTML/JSON。
8. 随后做 EPUB 和 PDF 文本提取；扫描 PDF/OCR、音频、YouTube 放到可选能力，不进入核心 MVP。
9. 长期扩展超广格式矩阵，并为每个新增格式建立质量基准、样例集和快照测试。
10. 后期推进 GUI/PWA/桌面外壳，但必须复用 Web 核心并保持本地优先处理。
