...
- [ ] 建立 Tauri 桌面壳，明确最小文件系统权限和禁用远程文档处理边界。
- [x] 明确 Tauri + Web-GUI 选择判断、体验目标和当前不足：见 `docs/DESKTOP_APP_ARCHITECTURE.md`。
- [ ] 将当前转换核心迁移为桌面 GUI 可复用模块，保持浏览器 smoke test 可继续运行。
- [ ] 建立文件拖拽、文件队列、任务状态、批量选择和失败重试。
- [ ] 建立输出目录选择和导出命名策略。
- [x] Worker 转换任务已建立基础链路。
- [x] 任务进度、取消、失败状态和 Blob URL 清理已建立浏览器基线。
- [x] local-only 安全状态已在当前前端展示。
- [ ] 建立 Input / DocumentModel / Output 三栏专业预览，并为窄屏切换为 tabs。
- [ ] 建立底部 Warnings / Quality Report / Diff / Versions 面板。
- [ ] 建立 Plugin Manager 和 Security Center 的桌面入口。
- [ ] 建立插件下载板块和更新板块入口，浏览器端和桌面端都必须可访问。

验收门槛：

- [ ] 桌面应用可启动、可选文件、可转换、可导出。
- [ ] 不需要联网即可完成基础热门格式转换。
- [ ] 文件系统权限只覆盖用户明确选择的输入文件和输出目录。
...
- [ ] 尚未建立真正的 Tauri 桌面壳、桌面文件权限和安装包流程。
- [ ] 前端仍是工作台基线，缺少成熟桌面软件的文件队列、任务中心、底部报告面板、版本历史、插件管理和安全中心。
